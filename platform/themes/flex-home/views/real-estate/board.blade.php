@php
    Theme::asset()->add('board-inline-css', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css');
@endphp

<style>
    :root {
        --col-properties: #3b82f6;
        --col-interested: #f59e0b;
        --col-visited: #10b981;
        --col-discarded: #ef4444;
    }
    ...
    .board-header-section h1 { font-size: 1.5rem; margin: 0; font-weight: 700; }
    .board-header-section .board-meta { opacity: 0.8; font-size: 0.875rem; }
    ...
    .kanban-column[data-status="properties"] .kanban-column-header { background: var(--col-properties); }
    .kanban-column[data-status="interested"] .kanban-column-header { background: var(--col-interested); }
    .kanban-column[data-status="visited"] .kanban-column-header { background: var(--col-visited); }
    .kanban-column[data-status="discarded"] .kanban-column-header { background: var(--col-discarded); }
...
<div class="board-header-section">
    <div class="container-fluid px-4">
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-2">
            <div>
                <h1><i class="fas fa-clipboard-list me-2"></i>{{ $board->name }}</h1>
            ...
            <div class="d-flex gap-2">
                ...
                <a href="{{ $board->email_share_url }}" class="btn btn-light btn-sm">
                    ...
                    @forelse($column['properties'] as $property)
                        <div class="property-card"
                            ...
                            @if(!empty($property->formatted_images) && count($property->formatted_images))
                                <img src="{{ $property->formatted_images[0] }}" class="property-card-img" alt="{{ $property->name }}" loading="lazy">
                            @else
                                <img src="{{ RvMedia::getDefaultImage() }}" class="property-card-img" alt="{{ $property->name }}">
                            @endif
                            ...
                            @if($property->url)
                                <a href="{{ $property->url }}" target="_blank" class="property-card-link">
                                    <i class="fas fa-external-link-alt me-1"></i> {{ trans('plugins/real-estate::board.view_property') }}
                                </a>
                            @endif
                        </div>
                    @empty
        ...
        zone.addEventListener('drop', function(e) {
            ...
            if (oldColumn.querySelectorAll('.property-card').length === 0) {
                ...
                if (oldStatus !== 'properties') {
                    empty.innerHTML = '<i class="fas fa-arrows-alt mb-2 d-block"></i> Drag properties here';
                } else {
            ...
            .then(function(data) {
                if (data.error) {
                    ...
                    if (!emp && dropZone.querySelectorAll('.property-card').length === 0) {
